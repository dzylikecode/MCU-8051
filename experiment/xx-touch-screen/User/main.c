...
void TOUCH_Test(void) {
  ...
  TOUCH_Scan();
  if (xpt_xy.sta) {
    if (xpt_xy.lcdx > tftlcd_data.width) xpt_xy.lcdx = tftlcd_data.width - 1;
    if (xpt_xy.lcdy > tftlcd_data.height) xpt_xy.lcdy = tftlcd_data.height - 1;
    ...
    if (xpt_xy.lcdy > tftlcd_data.height - 20) {
      ...
      LCD_Fill(xpt_xy.lcdx - 2, xpt_xy.lcdy - 2, xpt_xy.lcdx + 2,
               xpt_xy.lcdy + 2, penColor);
    }
  }
}
...
void GUI_Test(void) {
  ...
  LCD_ShowString(tftlcd_data.width / 2 - 7 * 12, 30, tftlcd_data.width,
                 tftlcd_data.height, 24, "www.prechin.cn");
  LCD_ShowString(tftlcd_data.width / 2 - 7 * 8, 55, tftlcd_data.width,
                 tftlcd_data.height, 24, "GUI Test");
...
Start:
  ...
  LCD_ShowString(tftlcd_data.width / 2 - 7 * 12, 30, tftlcd_data.width,
                 tftlcd_data.height, 24, "www.prechin.cn");
  FRONT_COLOR = RED;
  LCD_ShowString(tftlcd_data.width - 3 * 12, 0, tftlcd_data.width,
                 tftlcd_data.height, 24, "RST");
